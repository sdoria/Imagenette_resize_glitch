...
def get_data(size, woof, bs, workers=None, full_size=False):
    if   size<=128: path = URLs.IMAGEWOOF_160 if woof else URLs.IMAGENETTE_160
    elif size<=224: path = URLs.IMAGEWOOF_320 if woof else URLs.IMAGENETTE_320
    else          : path = URLs.IMAGEWOOF     if woof else URLs.IMAGENETTE
    
    
    if full_size: path = URLs.IMAGEWOOF     if woof else URLs.IMAGENETTE
...
def train(
        ...
        full_size: Param("Use full size dataset", int)=0,
        ):
    ...
    data = get_data(size, woof, bs, full_size=full_size)
...
def main(
    ...
    acc = np.array(
        [train(gpu,woof,lr,size,alpha,mom,eps,epochs,bs,mixup,opt,arch,sa,sym,dump,lrfinder,log,sched_type,ann_start,full_size)
                for i in range(run)])
